...
namespace AmazonBBS.BLL
{
    ...
    /// 分享链接表
    /// </summary>
	public class ShareLinkBLL : Auto_ShareLinkBLL
    {
		public static ShareLinkBLL Instance
        {
            get { return SingleHepler<ShareLinkBLL>.Instance; }
        }
        ShareLinkDAL dal = new ShareLinkDAL();

        public ShareLink GetLinkByUserID(long userID)
        {
            return ModelConvertHelper<ShareLink>.ConverToModel(dal.GetLinkByUserID(userID));
        }
        ...
        /// <param name="id">主键</param>
        /// <returns></returns>
        public ShareLink GetModel(long id)
        {
            return GetItem(id);
        }
		#endregion	
		...
		//
        //    List<ShareLink> li = ModelConvertHelper<ShareLink>.ConvertToList(tab);
        ...
        public Paging SearchByRows(int pageIndex)
        {
            ...
            if (paging.RecordCount > 0)
            {
                ...
                var li = ModelConvertHelper<ShareLink>.ConvertToList(tab);
                //paging.SetDataSource(li);
            }
            return paging;
        }
