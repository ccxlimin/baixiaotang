...
namespace AmazonBBS.BLL
{
    ...
    /// 职位
    /// </summary>
    public class JobBLL : Auto_JobBLL
    {
        public static JobBLL Instance
        {
            get { return SingleHepler<JobBLL>.Instance; }
        }
        ...
        /// <returns></returns>
        public ResultInfo Create(Job model, SqlTransaction tran = null)
        {
        ...
        /// <returns></returns>
        public ResultInfo Update(Job model, SqlTransaction tran = null)
        {
            ResultInfo ri = new ResultInfo();
            if (Edit(model, tran))
            {
        ...
        //
        //    List<Job> li = ModelConvertHelper<Job>.ConvertToList(tab);
        ...
        public Paging SearchByRows(int pageIndex)
        {
            ...
            if (paging.RecordCount > 0)
            {
                ...
                var li = ModelConvertHelper<Job>.ConvertToList(tab);
                //paging.SetDataSource(li);
            }
            return paging;
        }
        ...
        {
            return ModelConvertHelper<Job>.ConvertToList(dal.GetJobType());
        }

        public JobViewModel GetJobs()
        {
            var list = SearchAll();
            var jobtypes = list.Where(a => { return a.IsJobType == 1; }).ToList();
            var jobs = list.Where(a => { return a.IsJob == 1; }).ToList();
