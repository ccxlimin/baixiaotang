...
namespace AmazonBBS.Common
{
    ...
    public class SessionHelper
    {
        ...
        public static void Set(string strSessionName, object objValue, int iExpires = 0, int iYear = 0)
        {
            HttpContext.Current.Session[strSessionName] = objValue;
            if (iExpires > 0)
            {
        ...
        /// </summary>
        /// <param name="strSessionName">Session对象名称</param>
        /// <param name="defaultValue">当无session对象时的默认值</param>
        /// <returns>Session对象值</returns>
        public static T Get<T>(string strSessionName, T defaultValue)
        {
            object value = HttpContext.Current.Session[strSessionName];
            if (value == null)
            {
                return defaultValue;
            }
