...
namespace AmazonBBS.DAL
{
    ...
    public class Auto_GiftFeeDAL
    {
        ...
        /// <returns></returns>
        public int Add(GiftFee model, SqlTransaction tran = null)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into GiftFee(");			
			strSql.Append(" GiftID,FeeName,FeeType,Fee,FeeCount )");
			strSql.Append(" values (");
			strSql.Append("@GiftID,@FeeName,@FeeType,@Fee,@FeeCount);select @@IDENTITY");
        ...
        /// <param name="model"></param>
        /// <returns></returns>
		public bool Update(GiftFee model, SqlTransaction tran = null)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update GiftFee set ");
			strSql.Append("GiftID=@GiftID,FeeName=@FeeName,FeeType=@FeeType,Fee=@Fee,FeeCount=@FeeCount");
		
			strSql.Append(" where GiftFeeId=@GiftFeeId");
			SqlParameter[] parameters = 
			{
				        new SqlParameter("@GiftID", model.GiftID),
        new SqlParameter("@FeeName", model.FeeName),
		...
		{			
			string sql="delete from GiftFee where GiftFeeId=@GiftFeeId";
			SqlParameter[] parameters = { new SqlParameter("@GiftFeeId", Id) };			
        ...
        {
            string sql = "SELECT * FROM GiftFee WHERE GiftFeeId=@GiftFeeId";
			SqlParameter[] parameters = { new SqlParameter("@GiftFeeId", Id) };
			if (tran == null)
            {
		...
		public int Count()
        {
            string sql="select count(*) from GiftFee";
        ...
        {
            string sql = "SELECT * FROM GiftFee ORDER BY GiftFeeId desc ";
        ...
        {
            string sql = "SELECT top " + number + " * FROM GiftFee ORDER BY GiftFeeId desc ";
            return SqlHelper.GetTable(sql);
        }	
        ...
        public DataTable Query(int pageIndex, int pageSize)
        {
            ...
            sb.Append(" select * from  (SELECT ROW_NUMBER() OVER(ORDER BY GiftFeeId desc) as rowid ,* FROM GiftFee) ");
